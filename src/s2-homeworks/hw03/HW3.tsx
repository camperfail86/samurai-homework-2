...
-не дописал тип- 3 - дописать типы и логику функции pureAddUserCallback и проверить её тестами
...
export type UserType = {
    _id: string // need to fix any ДОПИСАЛ
    name: string // need to fix any ДОПИСАЛ
}

export const pureAddUserCallback = (name: string, setUsers: any, users: Array<UserType>) => { // need to fix any
    const user = { _id: v1(), name} // СОЗДАЛ ЮЗЕРА
    setUsers([...users, user])
}

const HW3 = () => {
    const [users, setUsers] = useState<Array<UserType>>([]) // need to fix any
    // ДОПИСАЛ ТИПИЗАЦИЮ ЮЗ СТЕЙТ
    const addUserCallback = (name: string) => { // need to fix any ДОПИСАЛ
